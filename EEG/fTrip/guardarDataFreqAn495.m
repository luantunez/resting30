...
for i=1:23     
     if i~=3 && i ~= 14 && i ~= 16 && i ~= 18  && i ~= 15 && i~=17    %los sujetos que no sirven, si es distinto de estos, hacer el promedio de los datos hasta largo min, sino no, si es igual a alguno no entra                 
        ...
        dataOrig=eeglab2fieldtrip(EEG, 'preprocessing');  %convierto]
        data=dataOrig;
        data.time{1,1}=data.time{1,1}(1:largoMin);
        data.trial{1,1}=data.trial{1,1}(:,1:largoMin);
        ...
        cfg.t_ftimwin    = ones(length(cfg.foi),1);   % length of time window = 0.5 sec
        cfg.toi          = 0.5:0.5:343;                  % time window "slides" from -0.5 to 1.5 sec in steps of 0.05 sec (50 ms)
