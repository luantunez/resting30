...
for i=1:34 
     if i~=[18 28 30 4 16 32]     %los sujetos que no sirven, si es distinto de estos, hacer el promedio de los datos hasta largo min, sino no, si es igual a alguno no entra                 
        if find(i==buenosNom) ~=0
            ...
            if i~=buenosNom(end)
                charBuenosNom=[charBuenosNom,'sujF(',num2str(i),').suj.freq,'];
            else
                charBuenosNom=[charBuenosNom,'sujF(',num2str(i),').suj.freq'];
            end
        elseif find(i==malosNom) ~=0
            sujF(i)=load([direcc,'\sujNormalizados\malosNom\s',num2str(i),'.mat']);
             if i~=malosNom(end)
                charMalosNom=[charMalosNom,'sujF(',num2str(i),').suj.freq,'];
            else
                charMalosNom=[charMalosNom,'sujF(',num2str(i),').suj.freq'];
             end
        end
        if find(i==buenosDef) ~=0
            ...
            if i~=buenosDef(end)
                charBuenosDef=[charBuenosDef 'sujF(',num2str(i),').suj.freq,'];
            else
                charBuenosDef=[charBuenosDef 'sujF(',num2str(i),').suj.freq'];
            end
        elseif find(i==malosDef) ~=0
            ...
            if i~=malosDef(end)
                charMalosDef=[charMalosDef,'sujF(',num2str(i),').suj.freq,'];
            else
                charMalosDef=[charMalosDef,'sujF(',num2str(i),').suj.freq'];
            end
        end
...
%      sujF(i).suj.freq.freq=mean(sujF(i).suj.freq.freq);
      sujF(i).suj.freq.time=mean(sujF(i).suj.freq.time);
      sujF(i).suj.freq.powspctrm=mean(sujF(i).suj.freq.powspctrm,3);
%      sujF(i).suj.freq.powspctrm=mean(sujF(i).suj.freq.powspctrm,2);
      sujF(i).suj.freq.powspctrm=squeeze(sujF(i).suj.freq.powspctrm);
      end
 end
...
cfg_neighb.elec      = sujF(1).suj.freq.elec;
...
if isfield(freqstNombres,'negclusters')
    imagesc(freqstNombres.posclusterslabelmat);
    title('Nombres posclusters');
    set(gca,'xTickLabel',frange);
    figure()
    imagesc(freqstNombres.negclusterslabelmat);
    title('Nombres negclusters');
    set(gca,'xTickLabel',frange);
end

if isfield(freqstDefiniciones,'negclusters')
    imagesc(freqstDefiniciones.posclusterslabelmat);
    title('Definiciones posclusters');
    set(gca,'xTickLabel',frange);
    figure()
    imagesc(freqstDefiniciones.negclusterslabelmat);
