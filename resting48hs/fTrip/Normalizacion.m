...
for i=1:23     
     if i~=3 && i ~= 14 && i ~= 16 && i ~= 18  && i ~= 15 && i~=17
        ...
        elseif find(i==malosNom.malosNom) ~=0
         ...
         sujPmedio=mean(suj.freq.powspctrm,3); %promedio en tiempos
         sujPmedio=mean(sujPmedio,2);     %promedio en frecuencias
         sujPmedio=mean(sujPmedio,1);    %promedio en canales
         
         suj.freq.powspctrm=suj.freq.powspctrm/sujPmedio;      %normalizo cada sujeto respecto a si mismo
         %.freq.pwrspctrm
         %los guardo
